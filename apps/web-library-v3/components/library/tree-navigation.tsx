...
function TreeNodeComponent({ node, level, parentPath = '' }: TreeNodeComponentProps) {
  const { expandedNodes, toggleNode, currentNodeId, setCurrentNode, language } = useAppStore()
    ...
    <div className="select-none">
      ...
      >
        ...
        <div className="flex flex-col min-w-0 flex-1">
          ...
          {node.sanskrit && isHovered && language !== 'hi' && (
    ...
    <ScrollArea className={cn("h-full", className)}>
      <div className="p-3">
        <div className="mb-3 px-2">
          <h2 className="text-sm font-semibold text-foreground">{t('vedic_library')}</h2>
          <p className="text-xs text-muted-foreground">{t('knowledge_os_desc')}</p>
        </div>
