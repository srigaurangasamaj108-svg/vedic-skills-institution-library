  ...
  return (
    <Card className="border-border/50 shadow-sm bg-card">
      ...
      <CardContent className="pt-0">
        <textarea
          placeholder={t('reflections_placeholder')}
        ...
        <div className="flex justify-between items-center mt-2">
          <span className="text-[10px] text-muted-foreground uppercase tracking-wider font-medium">{t('auto_saved')}</span>
          <Button variant="ghost" size="sm" className="h-7 text-xs">{t('clear')}</Button>
        </div>
      </CardContent>
    </Card>
  )
    ...
    <ScrollArea className={cn("h-full", className)}>
      <div className="p-4 space-y-4">
        <div className="px-1 mb-2 flex items-center justify-between">
          <div>
            <h2 className="text-sm font-semibold text-foreground">{t('apply')}</h2>
            <p className="text-xs text-muted-foreground">{t('for')}: {audience.charAt(0).toUpperCase() + audience.slice(1)}</p>
          </div>
